...
FACES = {
    "L": {"axis": 0, "pos": 2, "preview": lambda x: np.flip(x)},
    "R": {"axis": 0, "pos": 0, "preview": lambda x: np.flipud(x)},
    "D": {"axis": 1, "pos": 0, "preview": lambda x: np.rot90(x, -1)},
    "U": {"axis": 1, "pos": 2, "preview": lambda x: np.fliplr(np.rot90(x))},
    "F": {"axis": 2, "pos": 0, "preview": lambda x: np.rot90(np.flipud(x))},
    "B": {"axis": 2, "pos": 2, "preview": lambda x: np.rot90(x)},
}
...
class CubeSkeleton:
    ...
    def do_move(self, face, clockwise=True):
        ...
        for piece in self.cube[index].flatten():
            piece.ori[axis1], piece.ori[axis2] = piece.ori[axis2], piece.ori[axis1]
        return 0
